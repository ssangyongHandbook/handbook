...
public class PostController {
	...
	public ModelAndView list(@RequestParam(defaultValue = "0") int offset ,HttpSession session,
			...
			@RequestParam(required = false) String searchword,FollowingDto fdto) {
		...
		for(int i = 0; i<list.size(); i++) {
			...
			list.get(i).setChecklogin(pservice.checklogin(list.get(i).getPost_num(),(String)session.getAttribute("user_num")));
			list.get(i).setCheckfollowing(fservice.checkFollowing((String)session.getAttribute("user_num"),list.get(i).getUser_num()));
