...
<script src="https://kit.fontawesome.com/16085d762f.js" crossorigin="anonymous"></script>
...
<script type="text/javascript">
	$(function(){
		...
		//메신저 검색
		$(".messagesearch input").keyup(function(){
			setTimeout(function() {
				//메신저 검색이 입력됐을 때 현재 other_name에 저장된 이름과 같으면 실앵하지 않는다
					if(other_name!='' && other_name==$(".messagesearch input").val()){
						return;
						consol.log($(this).val());
					}else{
						memberListOut();
		...
		$('body').click(function(e){
			...
			if(container.css("display")!="none"){
				if (!$(e.target).hasClass('msgsearchuser') && !$(e.target).hasClass('msgadd') 
			    		&& !$(e.target).hasClass('msgaddicon') && !$(e.target).hasClass('msgaddname')
			    		&& !$(e.target).hasClass('msgsearchuser') && !$(e.target).hasClass('msgsearchuserone')) {
			...
			success:function(res){
				...
				$.each(res,function(i,ele){
					if(ele.sender_num=='${user_num}'){
						chatContent+="<div class='msgright msgone'><div class='msgdel' del="+ele.mess_num+">삭제</div><div class='messageright messagebubble'>" + ele.mess_content + "</div></div>";
					}else{
						chatContent+="<div class='msgleft msgone'><img src='"+otherImg+"' class='leftreceiverphoto'><div class='messageleft messagebubble'>" + ele.mess_content + "</div><div class='msgdel' del="+ele.mess_num+">삭제</div></div>";
					}
				})
		...
		$.ajax({
			...
			success:function(chatMember){
				...
				$.each(chatMember,function(i,chat){
					...
					out+='</div>';
					out+='<div class="messagememberinfo">';
					out+='<span class="membername" member_id='+chat.member_id+'>'+chat.member_name+'</span>';
					out+='<div class="chatdetail">';
